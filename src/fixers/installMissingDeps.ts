...
export async function installDependency(manager: "npm" | "pip", pkg: string, targetDir: string, progress?: Progress): Promise<FixResult> {
  ...
  return new Promise(resolve => {
    let child;
    try { child = spawn(command, args, { cwd: targetDir, windowsHide: true }); }
    catch (error) { resolve({ success: false, message: error instanceof Error ? error.message : "Could not start the installer." }); return; }
    let timedOut = false;
