...
export async function syncEnvVar(kind: string, targetDir: string, key?: string, value?: string, sourceKey?: string): Promise<FixResult> {
  ...
  if (kind === "env-file") {
    ...
    catch (error) { return { success: false, changed: false, message: error instanceof Error ? error.message : "Could not create .env." }; }
  }
  if (!key) return { success: false, changed: false, message: "No environment variable was supplied." };
  let current = "";
