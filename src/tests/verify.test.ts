...
test("an escalated repair does not take a verified repair down with it", async () => {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(target, "scripts", "preflight.js"),
      [
    ...
    await fs.writeFile(
      path.join(target, "scripts", "load-env.js"),
      [
...
test("onboard proves it never rewrote application code", async () => {
  const target = await copyFixture("crossfile-db-url-app");
  try {
    const codeBefore = await Promise.all(
      ["src/config.js", "src/db.js", "src/index.js", ".env.example", "deploy/platform.yaml"].map(
        async file => `${file}:${await fs.readFile(path.join(target, file), "utf8")}`,
      ),
    );
...
test("the onboard CLI reports time to green as JSON and exits 0", async () => {
  ...
  try {
    ...
    const payload = JSON.parse(stdout) as {
      ...
      install: { kind: string; offline: boolean; networkCalls: number };
      phases: Record<string, number>;
      remaining: unknown[];
      repair: { repairs: Array<{ status: string; repro: { command: string; before: { exitCode: number }; after: { exitCode: number } } }> };
...
test("the dashboard downloads the fixed copy for a locally served project", async () => {
  ...
  try {
    ...
    // downloadable copy…
    const scan = await (await fetch(`${running.url}/api/scan`)).json() as { uploaded: boolean; canDownload: boolean; displayName: string };
    ...
    assert.match(String((await tooEarly.json() as { error: string }).error), /run a repair first/i);
    ...
    const payload = JSON.parse(complete.split("\ndata: ")[1]) as { scan: { canDownload: boolean; canRevert: boolean } };
