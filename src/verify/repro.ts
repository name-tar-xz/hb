...
import { ReproEvidence, ReproRun, ReproSpec } from "../types.js";
...
export interface ReproOptions {
  ...
  /** Run the command through a shell (generated one-liners need this). */
  shell?: boolean;
  /** Secret values that must never survive into captured text. */
  secrets?: string[];
...
export function containsSecret(text: string, secrets: string[]): string[] {
...
export async function runRepro(options: ReproOptions): Promise<ReproRun> {
  ...
  return new Promise<ReproRun>(resolve => {
    const finish = (stdout: string, stderr: string, exitCode: number | null, timedOut: boolean) => {
    ...
    const collect = (target: "out" | "err") => (chunk: Buffer) => {
      ...
      if (target === "out") {
        if (stdout.length < 200_000) stdout += text;
      } else if (stderr.length < 200_000) stderr += text;
    };
...
export function runReproSpec(spec: ReproSpec, cwd: string, options: { timeoutMs?: number; secrets?: string[] } = {}): Promise<ReproRun> {
  return runRepro({
...
export function flippedToGreen(before: ReproRun, after: ReproRun): boolean {
...
export function failureMoved(before: ReproRun, after: ReproRun): boolean {
  return before.outputHash !== after.outputHash || before.exitCode !== after.exitCode;
}

export function evidence(run: ReproRun): ReproEvidence {
  return {
...
export function shortHex(hash: string, length = 12): string {
