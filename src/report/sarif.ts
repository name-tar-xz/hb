...
const RULE_BY_CATEGORY: Record<Diagnosis["category"], { id: string; name: string; description: string }> = {
...
const LEVEL: Record<Diagnosis["severity"], "error" | "warning" | "note"> = {
  error: "error",
  warning: "warning",
  info: "note",
};
...
export interface SarifOptions {
  ...
  /** Exit code of the run; recorded in `invocations` so CI can correlate. */
  exitCode?: number;
  runId?: string;
}
