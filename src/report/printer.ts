...
import { Diagnosis, Receipt, RepairRecord, ReproRun, ScanResult } from "../types.js";
...
function statusLine(repair: RepairRecord): string {
...
function reproBlock(label: string, run: ReproRun | undefined, expect: number): string {
  if (!run) return chalk.gray(`${label}: not run`);
 ...
 */
export function renderVerifiedReport(outcome: OnboardOutcome, expect = 0): string {
  const lines: string[] = [];
  ...
  if (outcome.repairs.length) {
    ...
    for (const repair of outcome.repairs) lines.push(statusLine(repair));
    lines.push("");
  }
...
function receiptHeadlinePlain(receipt: Receipt): string {
