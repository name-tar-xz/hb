...
import { Fingerprint, Policy, Severity } from "./types.js";
...
program.name("env-doctor")
  ...
  .action(async (input: string | undefined, options: Record<string, unknown>) => {
    ...
    const note = (line: string) => (options.json || options.sarif ? console.error(line) : console.log(line));
    ...
    if (options.onboard || options.verify) {
      try {
        ...
        if (options.json) console.log(JSON.stringify({ ...outcome, receipt: outcome.receipt }, null, 2));
    ...
    if (options.sarif || options.sarifOut) {
      ...
      const destination = options.sarifOut as string | undefined;
      if (destination) {
    ...
    if (options.dryRun) {
      ...
      for (const diagnosis of outcome.active.filter(d => d.autoFixable)) {
        if (repairClass === "env" && diagnosis.category !== "env") continue;
        if (!options.json) console.log(`\nFixing: ${diagnosis.title}`);
        const fixed = await fixDiagnosis(diagnosis, targetDir, line => { if (!options.json && line) process.stdout.write(`${line}\n`); });
        if (!options.json) console.log(fixed.success ? `✓ ${fixed.message}` : `✗ ${fixed.message}`);
      }
      result = await scanAll(targetDir);
      outcome = applyPolicy(result.diagnoses, policy);
      if (options.json) console.log(JSON.stringify({ ...result, diagnoses: outcome.active }, null, 2));
...
program.command("fingerprint")
  ...
  .action(async (input: string, options: { out?: string; diff?: string }) => {
    const targetDir = path.resolve(input);
    ...
    try {
      there = JSON.parse(await fs.readFile(path.resolve(options.diff), "utf8")) as Fingerprint;
    } catch (error) {
...
program.command("revert")
  ...
  .action(async (input: string, options: { session?: string; list?: boolean }) => {
    const targetDir = path.resolve(input);
    if (options.list) {
      const sessions = await listSessions(targetDir);
      if (!sessions.length) return void console.log("No repair sessions recorded.");
    ...
    for (const file of result.restored) console.log(chalk.gray(`  restored ${file}`));
