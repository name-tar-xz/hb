...
function parseEnvKeys(source: string): Map<string, string> {
...
async function lockfileDrift(targetDir: string): Promise<Fingerprint["lockfile"]> {
  ...
  const declared = { ...(manifest.dependencies as Record<string, string> ?? {}), ...(manifest.devDependencies as Record<string, string> ?? {}) };
  const packages = (lock.packages as Record<string, { version?: string }> | undefined) ?? {};
  const rows: Fingerprint["lockfile"] = [];
...
function satisfiesMajor(range: string, locked: string): boolean {
 ...
 */
export async function buildFingerprint(options: FingerprintOptions): Promise<Fingerprint> {
  ...
  const envRows: Fingerprint["env"] = [];
  for (const file of [".env.example", ".env"]) {
    try {
      const parsed = parseEnvKeys(await fs.readFile(path.join(targetDir, file), "utf8"));
      for (const [key, value] of parsed) keys.add(key);
      const present = new Map<string, string>();
      for (const [key, value] of parsed) present.set(key, value);
  ...
  try {
    for (const key of parseEnvKeys(await fs.readFile(path.join(targetDir, ".env.example"), "utf8")).keys()) exampleKeys.add(key);
  } catch {
    /* no template */
  }
  const localKeys = new Set<string>();
  try {
    for (const key of parseEnvKeys(await fs.readFile(path.join(targetDir, ".env"), "utf8")).keys()) localKeys.add(key);
  } catch {
    /* no local env */
  }
  ...
  const detected = options.policy?.verify.command ? undefined : detectVerifyCommand(manifest as { scripts?: Record<string, string> } | undefined);
 ...
 */
export function diffFingerprints(here: Fingerprint, there: Fingerprint): FingerprintDiff {
  const rows: FingerprintDiff = [];
  const runtimeKeys: Array<keyof Fingerprint["runtime"]> = ["node", "npm", "python", "platform", "arch"];
  const declaredKeys: Array<keyof Fingerprint["declared"]> = ["nvmrc", "enginesNode"];
  for (const key of declaredKeys) {
    const a = here.declared?.[key] ?? "(unset)";
    const b = there.declared?.[key] ?? "(unset)";
    if (a !== b) rows.push({ section: "declared", key, here: a, there: b });
  }
  for (const key of runtimeKeys) {
    const a = here.runtime[key] ?? "(absent)";
    const b = there.runtime[key] ?? "(absent)";
    if (a !== b) rows.push({ section: "runtime", key, here: a, there: b });
  }

  const lockIndex = (fingerprint: Fingerprint) => new Map(fingerprint.lockfile.map(row => [row.package, row]));
  ...
  const envIndex = (fingerprint: Fingerprint) => new Map(fingerprint.env.map(row => [row.key, row]));
