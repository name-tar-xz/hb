...
program.name("env-doctor").description("Diagnose local development environment problems.")
    ...
    if (options.dryRun) {
        ...
        for (const diagnosis of result.diagnoses.filter(d => d.autoFixable)) {
            ...
            const outcome = await fixDiagnosis(diagnosis, targetDir, line => { if (!options.json && line)
                process.stdout.write(`${line}\n`); });
            if (!options.json)
                console.log(outcome.success ? `✓ ${outcome.message}` : `✗ ${outcome.message}`);
        }
    ...
    if (result.diagnoses.some(d => d.severity === "error"))
        process.exitCode = 1;
});
