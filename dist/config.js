 ...
 */
export async function loadPolicy(targetDir, explicitPath) {
    ...
    for (const candidate of candidates) {
        ...
        try {
            ...
            if (explicitPath)
                throw new Error(`Could not read policy file ${candidate}: ${error instanceof Error ? error.message : "unreadable"}`);
            continue;
        }
        let parsed;
        try {
        ...
        if (failOn === "none" || failOn === "error" || failOn === "warning" || failOn === "info")
            policy.failOn = failOn;
        const ignore = raw.ignore;
        if (ignore !== undefined)
            policy.ignore = asStringArray(ignore);
        if (Array.isArray(raw.waivers)) {
            policy.waivers = raw.waivers.map((entry) => {
        ...
        if (typeof verify.command === "string")
            policy.verify.command = verify.command;
        if (typeof verify.expectExitCode === "number")
            policy.verify.expectExitCode = verify.expectExitCode;
        ...
        if (typeof receipt.out === "string")
            policy.receipt.out = receipt.out;
        return { ...policy, raw };
    }
    return policy;
}
...
export function isExpired(waiver, now = new Date()) {
    if (!waiver.expires)
        return false;
