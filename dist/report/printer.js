 ...
 */
export function renderOnboardReport(report) {
    const lines = [];
    ...
    const installDetail = report.install.kind === "skipped"
        ? chalk.gray("skipped")
        : `${report.install.exitCode === 0 ? chalk.green("✔") : chalk.red(`✖ exit ${report.install.exitCode}`)} ${chalk.gray(report.install.command ?? "")}${report.install.offline ? chalk.gray(" · offline") : chalk.gray(` · ${report.install.networkCalls} network call(s)`)}`;
    ...
    if (report.repair && report.repair.repairs.length) {
        ...
        for (const repair of report.repair.repairs) {
            ...
            lines.push(chalk.gray(`          exit ${repair.repro.before.exitCode} → ${repair.repro.after.exitCode}`));
            if (repair.rolledBack)
                lines.push(chalk.gray("          change reverted"));
        }
    }
    else {
    ...
    if (report.green) {
        ...
        for (const finding of report.remaining) {
            ...
            if (finding.repro)
                lines.push(chalk.gray(`      repro: ${finding.repro.command}`));
        }
    ...
    if (receipt) {
        ...
        if (report.receiptPath)
            lines.push(chalk.gray(`  written to ${report.receiptPath}`));
    }
    return lines.join("\n");
}
