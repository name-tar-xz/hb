...
export function sessionDir(targetDir, sessionId) {
...
export class Transaction {
    ...
    sessionId;
    label;
    constructor(targetDir, label, sessionId) {
    ...
    async snapshotAll(relativePaths) {
        for (const relative of relativePaths)
            await this.snapshot(relative);
    }
    get snapshotsTaken() {
...
export async function listSessions(targetDir) {
    try {
...
export async function readSession(targetDir, sessionId) {
    try {
...
export async function latestSession(targetDir) {
 ...
 */
export async function revertSession(targetDir, sessionId) {
    const manifest = sessionId ? await readSession(targetDir, sessionId) : await latestSession(targetDir);
    if (!manifest)
        return { success: false, message: "No repair session was found to revert.", restored: [] };
    const restored = [];
    ...
    for (const entry of manifest.entries) {
        ...
        if (observed !== entry.hashBefore)
            stillDirty.push(entry.file);
    }
