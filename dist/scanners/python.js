...
function parseRequirements(source) {
    ...
    for (const raw of source.split(/\r?\n/)) {
        ...
        if (match)
            parsed.push({ name: match[1], operator: match[2], version: match[3] });
    }
    return parsed;
}
...
function matches(version, operator, expected) {
...
export async function scanPython(targetDir) {
    let source;
    try {
        source = await fs.readFile(path.join(targetDir, "requirements.txt"), "utf8");
    }
    catch {
        return [];
    }
